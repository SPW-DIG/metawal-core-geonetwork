...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- 
    ...
    Same configuration as ISO19139 here.
      -->
  <xsl:template name="get-iso19139.rw-configuration">
    <xsl:copy-of select="document('config-editor.xml')"/>
  </xsl:template>


  <!-- Dispatch to the current profile mode -->
  <xsl:template name="dispatch-iso19139.rw">
    <xsl:param name="base" as="node()"/>
    <xsl:apply-templates mode="mode-iso19139.rw" select="$base"/>
  </xsl:template>


  <!-- The following templates usually delegates all to iso19139. -->
  <xsl:template name="evaluate-iso19139.rw">
    <xsl:param name="base" as="node()"/>
    <xsl:param name="in"/>
  ...
  <xsl:template name="evaluate-iso19139.rw-boolean">
    <xsl:param name="base" as="node()"/>
    <xsl:param name="in"/>
