      ...
      <xsl:for-each select="mdb:identificationInfo[1]/*[1]">
        <xsl:for-each select="mri:citation/cit:CI_Citation">
          ...
          <xsl:for-each select="cit:identifier/*[string(mcc:code/*)]">
            ...
            <!--  MW - Geoportail specific index  START -->
            <!-- localIdentifierRW -->
            <xsl:if test="not(matches(mcc:code/(gco:CharacterString|gcx:Anchor), '[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}'))">
      ...
      <xsl:for-each select="mdb:distributionInfo/*">
        ...
        <xsl:for-each select="$linkConfig/link">
          <xsl:variable name="gpLink"
                        select="current()"/>
          <xsl:for-each select="$allLinks[cit:linkage/gco:CharacterString != ''
            ...
            <xsl:for-each select="$tokens">
              ...
              <xsl:variable name="path"
                            select="string-join($tokens[
                                        position() &lt; $pos], ' - ')"/>
              <xsl:if test="$path != ''">
                <xsl:element name="{$roleField}_tree">
