//_modules_jquery_javascript_jquery.min.js_1453302840192.min.js
...
//_modules_assets_javascript_jquery.jahia.min.js_1524063985221.min.js
...
//_modules_bootstrap_javascript_bootstrap-dropdown.js_1453301229893.min.js
...
//_modules_bootstrap-components_javascript_twitter-bootstrap-hover-dropdown.js_1515171918969.min.js
...
//_modules_geoportail-metawal_javascript_js.cookie.js_1559829854541.min.js
/*!
...
//_modules_geoportail-metawal_javascript_metawal.js_1559829854541.min.js
$(function(){var f=$(document);var b=["service","notification","download"];var e="serviceId";$(document).on("change",".js-metadata-walonmap-select",function(h){var g=$(this);g.find("option:not(:selected)").each(function(j,k){var l=$(k).attr("value");removeFromWalonmapSelection(l);renderWalonmapButtons(l)});renderWalonmapCart();h.stopPropagation()});$(document).on("click",".js-metadata-walonmap-toggle",function(k){var j=$(this);var l=j.data("file-identifier");var h="input[data-file-identifier="+l+"]";var i=$(h);var g=$(h).not(this);if(isSelectedForWalonmap(l)){removeFromWalonmapSelection(l);if(j.data("post-message")){postMessageWalonmap(l)}if(!j.hasClass("js-metadata-walonmap-toggle--checkbox")){j.text(j.data("add-label"))}else{i.each(function(m,o){var n=$(o);n.text(n.data("add-label"));if(n.hasClass("js-metadata-walonmap-toggle--checkbox")){n.closest(".walonmap-checkbox").find(".walonmap-checkbox-icon").prop("title",n.data("add-label"))}});g.each(function(n,o){var m=$(o);m.prop("checked",function(p,q){return !q})})}}else{if(window.siteOptions.walonmapSelection.selectionLimit>getWalonmapCartSize()){addToWalonmapSelection(l);if(j.data("post-message")){postMessageWalonmap(l)}if(!j.hasClass("js-metadata-walonmap-toggle--checkbox")){j.text(j.data("remove-label"))}else{i.each(function(m,o){var n=$(o);n.text(n.data("remove-label"));if(n.hasClass("js-metadata-walonmap-toggle--checkbox")){n.closest(".walonmap-checkbox").find(".walonmap-checkbox-icon").prop("title",n.data("remove-label"))}});g.each(function(n,o){var m=$(o);m.prop("checked",function(p,q){return !q})})}}else{i.each(function(m,o){var n=$(o);if(n.hasClass("js-metadata-walonmap-toggle--checkbox")){n.prop("checked",false)}});g.each(function(n,o){var m=$(o);m.prop("checked",function(p,q){return !q})});showMessage(window.siteOptions.walonmapSelection.selectionLimitMessage)}}renderWalonmapCart();k.stopPropagation()});window.onmessage=function(h){if(typeof h==="object"){var i=h.data;if(i){var g=typeof i==="string"?JSON.parse(i):i;if(g.action==="removeData"){if(g[e]){removeFromWalonmapSelection(g[e]);renderWalonmapButtons()}}}}};f.on("click",".js-metadata-downloads-toggle",function(){var i=$(this);var j=i.data("metawal-id");var g=i.data("url");var h=i.data("action");postJson(g,{metawalId:j,action:h},function(){$('.js-metadata-downloads-toggle[data-metawal-id="'+j+'"]').each(function(k,l){c($(l))});$(".js-metadata-downloads-select").each(function(l,n){var k=$(n),m=k.find('option[value="'+j+'"]');if(h==="add"){if(m.length===0){k.append('<option value="'+j+'" selected>'+i.data("title")+"</option>")}else{m.prop("selected",true)}}else{if(h==="remove"){m.remove()}}k.trigger("chosen:updated")});d()},function(k){if(k&&k.responseJSON&&k.responseJSON.error==="limit"){showMessage(window.siteOptions.downloadCartLimitMessage)}})});f.on("change",".js-metadata-downloads-select",function(){var g=$(this);g.find("option:not(:selected)").each(function(h,k){var j=$(k),l=j.attr("value");postJson(g.data("url"),{metawalId:l,action:"remove"},function(){$('.js-metadata-downloads-toggle[data-metawal-id="'+l+'"]').each(function(m,o){var n=$(o);c(n)});j.remove();g.trigger("chosen:updated");d()})})});function d(){var g=$(".js-metadata-downloads-select");$(".js-metadata-downloads-count").text("("+g.find("option:selected").length+")")}f.on("click",".js-metadata-notifications-manage",function(){var h=$(this);var g=h.data("metawal-id");postJson(h.data("url"),{metawalId:h.data("metawal-id"),action:"remove"},function(){$("#notifications"+g).remove();var j=$(".results-count--notifications");j.html(j.html()-1);var i=h.closest("li");if(i.length!==0){var k=$(".results-count--notifications-detail--"+i.attr("class"));k.html(k.html()-1)}})});f.on("click",".js-metadata-notifications-toggle",function(){var g=$(this);postJson(g.data("url"),{metawalId:g.data("metawal-id"),action:g.data("action")},function(h){c(g);$(".js-metadata-notifications-count").text("("+h.itemsNumber+")")})});f.on("click",".notifications-list-menu > li",function(){a($(this).data("value"),true)});f.on("change",".notifications-list-select > select",function(){a($(this).val())});function a(g,h){$(".notifications-list-menu > li").removeClass("active");$('.notifications-list-menu > li[data-value="'+g+'"]').addClass("active");$(".notifications-list-content > li").hide();$('.notifications-list-content > li[class="'+g+'"]').show();if(h){$(".notifications-list-select > select").val(g).trigger("chosen:updated")}}f.on("click",".js-metadata-services-manage",function(){var g=$(this);var h=g.data("metawal-id");postJson(g.data("url"),{metawalId:h,action:"remove"},function(){var i=$(".results-count--services");i.html(i.html()-1);$("#services"+h).remove()})});f.on("click",".js-metadata-services-toggle",function(){var g=$(this);postJson(g.data("url"),{metawalId:g.data("metawal-id"),action:g.data("action")},function(h){c(g);$(".js-metadata-services-count").text("("+h.itemsNumber+")")})});f.on("click",".invalidate-btn",function(){var g=$(this);postJson(g.data("url"),undefined,function(){g.prop("disabled",true);g.text(g.data("alt-text"))},function(){console.error("Error while trying to invalidate the following metadata : "+g.parent().parent().attr("id"))})});f.on("click",".post-btn",function(){var h=$(this),i=h.text(),g=h.data("alt-text");h.attr("disabled","disabled");if(g){h.text(g)}postJson(h.data("url"),undefined,function(){h.removeAttr("disabled");if(g){h.text(i)}})});function c(g){var h=g.text();g.text(g.data("alt-text"));g.data("alt-text",h);g.data("action",(g.data("action")==="add"?"remove":"add"))}b.forEach(function(g){var i="metadata-"+g+"-id";var h=Cookies.get(i);if(h){if(window.geoportail&&window.geoportail.logged){postJson(window.geoportail.homePath+".manage"+g.capitalizeFirstLetter()+".do",{metawalId:h,action:"add"},function(j){c($(".js-metadata-"+g+"s-toggle[data-metawal-id='"+h+"']"));$(".js-metadata-"+g+"s-count").text("("+j.itemsNumber+")");if(g==="download"){$("#dl-form").parent().load(window.geoportail.mainResource.path+".html #dl-form",function(){$("#dl-form").initComponents()})}})}else{console.error("Not connected !!")}Cookies.remove(i)}f.on("click",".js-metadata-"+g+"s-toggledisabled",function(){Cookies.set(i,$(this).data("metawal-id"));$("#connect").click()})});convertWalonmapCookies();renderWalonmapButtons();renderWalonmapCart();bindEventForCatalog()});function getWalonmapCookie(){return Cookies.get("walonmapCart")}function setWalonmapCookie(a){Cookies.set("walonmapCart",a)}function convertWalonmapCookies(){var a=Cookies.get("walonmapSelection");if(a&&a.length){getWalOnMapInformation("nodeIdentifiers",a,function(b){var c=b.cart;if(c&&c.length){$.each(c,function(d,e){if(e.fileIdentifier){addToWalonmapSelection(e.fileIdentifier)}})}});Cookies.remove("walonmapSelection")}}function getWalonmapCartSize(){var a=getWalonmapCookie();if(a){if(a.endsWith(";")){return a.split(";").length-1}else{return a.split(";").length}}else{return 0}}function isSelectedForWalonmap(b){var a=getWalonmapCookie();return a&&a.indexOf(b)>-1}function removeFromWalonmapSelection(a){var b=getWalonmapCookie();if(b){setWalonmapCookie(b.replace(a+";",""))}}function addToWalonmapSelection(a){if(!isSelectedForWalonmap(a)){setWalonmapCookie((getWalonmapCookie()||"")+a+";")}}function renderWalonmapButtons(b){var a=getWalonmapCookie();$(".js-metadata-walonmap-toggle"+(b?"[data-file-identifier='"+b+"']":"")).each(function(e,g){var c=$(g);var d=c.data("file-identifier");if(d){var h=a&&a.indexOf(d)>-1;var f=h?c.data("remove-label"):c.data("add-label");c.text(f);if(c.data("selected")==undefined){c.attr("data-selected",h)}else{c.data("selected",h)}if(c.hasClass("js-metadata-walonmap-toggle--checkbox")){c.prop("checked",h);c.closest(".walonmap-checkbox").find(".walonmap-checkbox-icon").prop("title",f)}}})}function renderWalonmapCart(){var b=$("dt[data-cookie-name='walonmap']");var a=$("dd[data-cookie-name='walonmap']");if(b.length&&a.length){getWalOnMapSelection(function(d){var c=d.cart;emptyWalonmapSelect();if(c&&c.length>0){b.addClass("is-toggle").find(".js-metadata-walonmap-count").text(" ("+c.length+")");a.show();$.each(c,function(e,f){addOptionToWalonmapSelect(f.fileIdentifier,f.label)})}else{b.removeClass("is-toggle").find(".js-metadata-walonmap-count").text(" (0)");a.hide()}})}}function emptyWalonmapSelect(){$(".js-metadata-walonmap-select").empty()}function addOptionToWalonmapSelect(a,b){$(".js-metadata-walonmap-select").each(function(d,e){var c=$(e),g=c.find('option[value="'+a+'"]');if(g.length===0){var f=$("<option/>");f.attr({value:a,selected:true}).text(b);c.append(f)}else{g.prop("selected",true)}c.trigger("chosen:updated")})}function showMessage(b){var a=$("#limit-popup",window.top.document);if(a.length){a.find("#limit-popup-message").text(b);a.show()}else{window.alert(b)}}function getWalOnMapSelection(a){getWalOnMapInformation("fileIdentifiers","cookie",a)}function getWalOnMapInformation(d,b,a){var c={};c[d]=b;$.ajax({type:"GET",contentType:"application/json",url:window.geoportail.homePath+".getWalOnMapCart.do",data:c,dataType:"json",success:a,async:false})}function postMessageWalonmap(a){getWalOnMapInformation("fileIdentifiers",a,function(b){var c=b.cart;if(c&&c.length){window.parent.postMessage(JSON.stringify(c[0]),"*")}else{removeFromWalonmapSelection(a);$(".js-metadata-walonmap-toggle--checkbox[data-file-identifier='"+a+"']").attr("checked",false);console.log("Unable to retrieve information from this ID : "+a)}})}function postJson(c,d,b,a){$.ajax({url:c,data:d,method:"post",dataType:"json",success:b,error:a?a:defaultErrorHandler})}function resetMetadataLoginCookies(){["service","notification","download"].forEach(function(a){Cookies.remove("metadata-"+a+"-id")})}function setLimitSettings(c,b,d){var a={selectionLimit:c,selectionLimitMessage:b};if(!window.siteOptions){window.siteOptions={walonmapSelection:a,downloadCartLimitMessage:d}}else{window.siteOptions.walonmapSelection=a;window.siteOptions.downloadCartLimitMessage=d}}function defaultErrorHandler(a){console.error("Erreur "+(a?("("+a.status+" - "+a.statusText+") : "+(a.responseText?JSON.parse(a.responseText).error:"inconnue...")):"inconnue..."))}function showMetadataDescription(h,e,a,c,d,g){$("body").css("cursor","progress");var b="mdDialog";var f=$("#mdDialog");if(f.length===0){$("#content .grid_12 .mainContentWrapper").after('<div id="mdDialog"></div>')}if(d){b=b+"_"+d;$("#mdDialog").after('<div id="'+b+'"></div>')}else{$("#"+b).hide().empty()}$.get(e,function(i){$(i).find("#metawalFullPage").appendTo("#"+b);$("#"+b+" .ui-tabs").tabs();if(a){$("#"+b+" .ui-tabs").tabs("select",a)}if(typeof _gaq!=="undefined"&&_gaq!==null){_gaq.push(["_trackPageview",e])}$("body").css("cursor","auto");$("#"+b).dialog({minWidth:600,width:700,modal:true,resizable:false,title:"<h2>"+h+"</h2>"});$("#dialogUrl_"+g).dialog({autoOpen:false,resizable:false,height:200,width:500,modal:true,position:["top",50]});$("#"+b+" .metaContent").each(function(){if($(this).text().trim()==""&&$(this).find("li").length==0){$(this).text(c);$(this).addClass("infoNA")}});$("#"+b+" .metaContentMotsCles").each(function(){if($(this).text().trim()==""&&$(this).find("li").length==0){$(this).text(c);$(this).addClass("infoNA")}});$("#"+b+" .metaContentMotsCles2").each(function(){if($(this).text().trim()==""&&$(this).find("li").length==0){$(this).text(c);$(this).addClass("infoNA")}});$("div.ui-tabs-panel").each(function(){$(this).find("div.box").last().addClass("last")})})}if(!String.prototype.capitalizeFirstLetter){String.prototype.capitalizeFirstLetter=function(){return this.charAt(0).toUpperCase()+this.slice(1)}}if(!String.prototype.endsWith){String.prototype.endsWith=function(c,b){var a=this.toString();if(typeof b!=="number"||!isFinite(b)||Math.floor(b)!==b||b>a.length){b=a.length}b-=c.length;var d=a.lastIndexOf(c,b);return d!==-1&&d===b}}function bindEventForCatalog(){var b=$(document);b.on("click",'.walonmap--subTitle[data-dynamic="true"]',function(){var g=$(this);var i=g.data("theme");var h=g.data("subtheme");var j=g.data("text");var f=g.closest(".result-item.is-accordion.is-accordion--walonmap");var e=f.find("#jsAjaxContainer_"+i+"_"+h);var d=f.find(".Loader__loading");if(f.hasClass("is-close")||f.hasClass("is-toggle")){$.ajax({url:window.siteOptions.catalogSettings.catalogUrl,type:"GET",data:{"search-themes":h,"search-text":j},success:function(l){d.slideUp();if(e.length>0){e.html(l).slideDown();var k=e.find(".btn.btn-none.sprite.sprite-info-blue.dialog-btn-metadata.js-walonmapInfoMoreButton");if(!k.length){e.html("<div class='is-no-result'>"+window.siteOptions.catalogSettings.catalogNoResultMessage+"</div>")}renderWalonmapButtons();g.removeData("dynamic");g.removeAttr("data-dynamic")}}})}});b.on("click",".js-walonmapInfoMoreButton",function(){var d=$(this).closest(".js-result-item").next(".walonmapInfo");$(d).stop(true,false).slideToggle()});b.on("click",".result-item.is-accordion h3.walonmap--title",function(h){var g=$(this),f=g.closest(".result-item.is-accordion"),e=f.find(".result-item-content:not(.result-item-content--subContent)"),d=$(h.target);if(d.is("a")){return}if(e.is(":not(:visible)")){e.hide();f.removeClass("is-toggle")}else{f.addClass("is-toggle")}f.removeClass("is-close");e.slideToggle()});b.on("click",".result-item.is-accordion h4.walonmap--subTitle",function(h){var g=$(this),f=g.closest(".result-item.is-accordion"),e=f.find(".result-item-content"),d=$(h.target);if(d.is("a")){return}if(e.is(":not(:visible)")){e.hide();f.removeClass("is-toggle")}else{f.addClass("is-toggle")}f.removeClass("is-close");e.slideToggle()});var a=$("#searchKeywords").val();var c=$("#searchKeywordsMobile").val();if((a&&a.length)||(c&&c.length)){$(".walonmap--title").click()}$(".result-item--theme").each(function(d,g){var f=$(g);if(!f.find(".walonmap--subTitle").length){f.remove()}})}function setCatalogSettings(b,a){if(!window.siteOptions){window.siteOptions={}}window.siteOptions.catalogSettings={catalogUrl:b,catalogNoResultMessage:a}};;
//_modules_geoportail-metawal_javascript_paginator-metawal.js_1559829854541.min.js
$(function(){var f=$(document);var e={};f.on("click",".results .paginator-navigation .btn[data-page]",function(h,j){var g=$(this);var s=g.closest(".paginator-navigation");var m=s.closest(".toolbar");var p=g.data("page");switch(p){case"start":p=s.find(".is-start .btn").data("page");break;case"prev":p=s.find(".paginator-page.active").prev().find(".btn").data("page");break;case"next":p=s.find(".paginator-page.active").next().find(".btn").data("page");break;case"end":p=s.find(".is-end .btn").data("page");break;default:break}var o=s.data("target");var i=s.data("url");var q=s.data("pageSize");var l=(p-1)*q;var n=(p*q)-1;a(o,i,l,n);d(o,p);if(!j){var t="page-"+s.data("type");c(t,p)}if(m.is(".toolbar-bottom")){var r=$(".result-filters").offset().top;var k=f.scrollTop();if(r<k){f.scrollTop(r)}}});function a(k,i,j,h){var g=$(k);if(typeof e[k]!=="undefined"){e[k].abort();delete e[k];g.removeClass("result-pages--loading")}g.addClass("result-pages--loading");e[k]=$.ajax({url:i,data:{begin:j,end:h},dataType:"html"});e[k].done(function(l){delete e[k];g.html(l);renderWalonmapButtons();renderWalonmapCart();g.removeClass("result-pages--loading")});e[k].fail(function(l){delete e[k];g.removeClass("result-pages--loading")})}function d(i,g){var h=$('.results .paginator-navigation[data-target="'+i+'"]');h.each(function(){var m=$(this);var k=m.closest(".paginator").find(".paginator-current");var l=m.data("pagesAround");var j=m.data("pagesCount");var n=m.data("showPrevAfter");var r=m.data("showNextBefore");var o=k.data("template");k.html(o.replace("{current}",g));var q=g-l;var p=g+l;if(g==1){p=p+1}if(g==j){q=q-1}m.find(".paginator-page.active").removeClass("active");m.find(".paginator-page").css("display","none");m.find('.btn[data-page="'+g+'"]').closest(".paginator-page").addClass("active");m.find(".paginator-page").each(function(){var s=$(this);var t=s.find(".btn").data("page");if(t>=q&&t<=p){s.css("display","")}});if(g>n){m.find(".paginator-start, .paginator-previous").removeClass("is-disabled")}else{m.find(".paginator-start, .paginator-previous").addClass("is-disabled")}if(g<r){m.find(".paginator-end, .paginator-next").removeClass("is-disabled")}else{m.find(".paginator-end, .paginator-next").addClass("is-disabled")}if(g<=l*2){m.find(".paginator-ellipsis-starting").css("visibility","hidden")}else{m.find(".paginator-ellipsis-starting").css("visibility","")}if(g>(j-l*2)){m.find(".paginator-ellipsis-ending").css("visibility","hidden")}else{m.find(".paginator-ellipsis-ending").css("visibility","")}})}function c(i,h){if(window.history&&window.history.replaceState){var g=window.location.pathname+"?"+b(document.location.search,i,h);if(window.location.hash.length){g+=window.location.hash}window.history.replaceState(null,null,g)}}function b(l,k,m){k=encodeURIComponent(k);m=encodeURIComponent(m);var h=l.substr(1).split("&");var j=h.length;var g;while(j--){g=h[j].split("=");if(g[0]==k){g[1]=m;h[j]=g.join("=");break}}if(j<0){h[h.length]=[k,m].join("=")}return h.join("&")}setTimeout(function(){$(window).on("focus",function(){var g=$(".results .paginator-navigation .paginator-page.active .btn");g.trigger("click",[true])})},3000)});;
//_modules_assets_javascript_jquery-ui.min.js_1524063985221.min.js
...
//_modules_templates-geoportail_javascript_jquery.form.js_1559829814816.min.js
/*!
 * jQuery Form Plugin
...
//_modules_assets_javascript_jquery.validate.js_1524063985221.min.js
...
//_modules_templates-geoportail_javascript_popups.js_1559829814816.min.js
function showModalAlert(b,c){var a=$("#geoportail-modal-alert");a.find(".alert-title").html(c);a.find(".alert-message").html(b);showModalBackdrop();a.removeClass("hide");a.addClass("in");a.find(".alert-popup-close-btn").focus()}function hideModalAlert(){var a=$("#geoportail-modal-alert");a.removeClass("in");a.addClass("hide");hideModalBackdrop()}function showModalBackdrop(){$("#geoportail-modal-backdrop").addClass("visible")}function hideModalBackdrop(){$("#geoportail-modal-backdrop").removeClass("visible")}$(document).on("click",".alert [data-dismiss]",function(){$($(this).data("dismiss")).hide()});;
//_modules_templates-geoportail_javascript_vendor.js_1559829814816.min.js
