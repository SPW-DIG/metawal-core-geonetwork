          ...
          <xsl:when
            ...
            <div class="attr">
              <div title="{/root/gui/strings/editAttributes}"
                   onclick="toggleFieldset(this, Ext.getDom('toggled{$id}'));" style="display: none;">
                <xsl:attribute name="class">
                  <xsl:choose>
          ...
          <legend id="stip.{$helpLink}|{$id}">
            ...
            <span class="toggle">
              <xsl:if test="/root/gui/config/metadata-view-toggleTab">
                <xsl:attribute name="onclick">toggleFieldset(Ext.getDom('toggled-bt-<xsl:value-of select="$id"/>'),
                  Ext.getDom('toggled<xsl:value-of select="$id"/>'));
                </xsl:attribute>
  ...
  <xsl:template name="hiddenElement">
    ...
    <xsl:call-template name="simpleElementGui">
      ...
      <xsl:with-param name="text">&#160;
        <img class="helplink" id="{generate-id()}{name(.)}|hidden-elements" src="{/root/gui/url}/images/important.png"/>
      </xsl:with-param>
      <xsl:with-param name="helpLink" select="$helpLink"/>
    </xsl:call-template>
  </xsl:template>
