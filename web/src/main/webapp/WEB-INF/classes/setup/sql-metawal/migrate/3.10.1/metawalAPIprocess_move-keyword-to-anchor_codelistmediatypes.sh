...
#curl -X GET "$CATALOG/srv/fre/q?any=cac0276e-a75e-4830-afcc-6c82bb70fc63&_isTemplate=y+or+n&summaryOnly=true&bucket=111" -H "X-XSRF-TOKEN: $TOKEN" -c /tmp/cookie -b /tmp/cookie --user $CATALOGUSER:$CATALOGPASS
...
curl -X POST "$CATALOG/srv/api/0.1/processes/move-keyword-to-anchor?bucket=111&updateDateStamp=false&thesaurusFileName=httpregistrymetawalcodelistmediatypes-media-types" -H "accept: application/json" -H "X-XSRF-TOKEN: $TOKEN" -c /tmp/cookie -b /tmp/cookie --user $CATALOGUSER:$CATALOGPASS
